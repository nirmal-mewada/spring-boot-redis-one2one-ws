...
public class SubscribeMessageHandler implements RedisPubSubListener<String, String> {
    private WebSocketSessionManager webSocketSessionManager;

    private static final Logger logger = LoggerFactory.getLogger(SubscribeMessageHandler.class);

    public SubscribeMessageHandler(WebSocketSessionManager webSocketSessionManager){
        this.webSocketSessionManager = webSocketSessionManager;
    }
    @Override
    public void message(String channel, String message) {
        var ws = this.webSocketSessionManager.getWebSocketSessions(channel);
        logger.info("got the message on redis "+ channel+ " and "+ message + ", ws: "+ws);
        try {
